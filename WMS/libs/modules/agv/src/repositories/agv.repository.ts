...
import { IAGV, CreateAGVDTO, AGVStatus } from '../../../master-data/src/master-data.model';
...
export class AGVRepository extends BasePostgresRepository {
    ...
    async createAGVWithClient(client: PoolClient | null, data: CreateAGVDTO & { current_x: number, current_y: number }): Promise<IAGV> {
        const query = `
            INSERT INTO agv (
